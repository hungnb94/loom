...
class ConditionNode(BaseNode):
    ...
    def _safe_eval(expr: str):
        ...
        return ConditionNode._eval_node(tree.body)
    ...
    def _eval_node(node: ast.AST):
        ...
        if isinstance(node, ast.UnaryOp):
            ...
            return op(ConditionNode._eval_node(node.operand))
        if isinstance(node, ast.BinOp):
            op = _SAFE_OPS.get(type(node.op))
            if not op:
                raise _SafeEvalError(f"Unsupported binary op: {type(node.op).__name__}")
            return op(ConditionNode._eval_node(node.left), ConditionNode._eval_node(node.right))
        if isinstance(node, ast.BoolOp):
            values = [ConditionNode._eval_node(v) for v in node.values]
            op = _SAFE_OPS.get(type(node.op))
            if not op:
        ...
        if isinstance(node, ast.Expression):
            return ConditionNode._eval_node(node.body)
